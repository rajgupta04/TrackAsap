      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <GlassCard padding="p-6 md:p-8" className="lg:col-span-5 flex flex-col justify-between h-full">
          <div>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="text-lg font-bold text-white">Share Hub</h2>
                  <p className="text-xs text-dark-400">Notes, sheets & community resources</p>
                </div>
              </div>
              <Link
                to="/hub"
              ...
              {postsLoading ? (
                ...
                <div className="space-y-3">
                  {posts.slice(0, 4).map((post, index) => {
                    ...
                    const authorName = author?.name || author?.username || author?.handle || 'Anonymous Member';
                    ...
                    return (
                      ...
                      >
                        ...
                        {post.title && (
                          <h4 className="text-xs font-bold text-white mb-1 group-hover:text-neon-green transition-colors">
                            {post.title}
                          </h4>
                        )}
                        ...
                        {post.attachment?.fileName && (
                          <div className="mt-2 inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-[10px] font-semibold">
                            <FileText className="w-3 h-3" /> {post.attachment.fileName}
                          </div>
                        )}

                        {post.sharedSheetSnapshot?.name && (
                          <div className="mt-2 inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg bg-neon-green/10 border border-neon-green/30 text-neon-green text-[10px] font-semibold">
                            <BookOpen className="w-3 h-3" /> {post.sharedSheetSnapshot.name}
                          </div>
                        )}
          ...
          <div className="pt-4 border-t border-white/5 mt-auto">
            <Link
              to="/hub"
              className="flex items-center justify-between text-xs font-semibold text-dark-400 hover:text-white transition-colors group"
            >
              <span>Explore All Resources in Share Hub</span>
